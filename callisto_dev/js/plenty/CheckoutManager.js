(function ($) {
     ...
     *
     * @param errorMessages - Array of error messages objects. example: [{code: 400, message: "something didn't work as
     *     it should!"}]
     */
        ...
        return $.ajax(
            "/rest/checkout/",
            {
                ...
                success: function (response) {
					if (!!response) {
						CheckoutManager.checkout = response.data;
					}
					else {
						CheckoutManager.throwError(0,
												   'Could not receive checkout data [GET "/rest/checkout/" receives null value]');
					}
                }
            }
        );
    ...
    CheckoutManager.setCustomerSignAndInfo = function () {
		...
		if (!CheckoutManager.checkout.CheckoutCustomerSign) {
			CheckoutManager.checkout.CheckoutCustomerSign = "";
		}
		if (!CheckoutManager.checkout.CheckoutOrderInfoText) {
			CheckoutManager.checkout.CheckoutOrderInfoText = "";
		}
                ...
                "/rest/checkout/placeorder/",
                {
                    ...
                    success: function (response) {
						...
						if (response.data.MethodOfPaymentRedirectURL != '') {
							...
							if (response.data.MethodOfPaymentAdditionalContent != '') {
								$('[data-plenty-id="dynamicModal"]').prepareModal({
